...
2. 再看 MCP 是否可用；若不可用，先走 `MCP setup / verify` 路径。
3. 再看 `app-compete.md` 是否声明了：
   - `target_app`
   - `platforms`
   - `default_market`
4. 根据用户意图路由：
...
- 没装好 MCP 时，任何分析都必须暂停，并引用 `mcp-installation.md`。
...
- 用户要求“月报 / 上个月对比 / 美国市场月度变化”时，reporting 默认使用 `monthly-deep-dive`。
